...
{ 
    ...
    { 
        ...
        if (!strncmp(buff, "1", 1))
        {
            ...
            manual_packet.idx = root["light"].asUInt();
            manual_packet.percent = root["dim"].asUInt();
            // send_struct(MSP_EX_DIM_ADD,&manual_packet,sizeof(manual_packet));
            while(1)
            {
                // serial_get_buffer();
                if (flag_message)
        ...
        // {
        //     reader.parse(buff, root);
        //     cout << "light" << root["light"].asString() << "dim" << root["dim"].asString() << endl;
        //     manual_packet.idx = root["light"].asUInt();
        //     manual_packet.percent = root["dim"].asUInt();
        //     send_struct(MSP_EX_DIM_ADD,&manual_packet,sizeof(manual_packet));
        //     while(1)
